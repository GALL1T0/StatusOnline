...
class DashboardController extends Controller
{
    ...
    public function reparacionesRealizadas()
    {
        ...
        if ($reparaciones_realizadas_1_semana != 0) {
            ...
            $porcentaje_reparacion_realizada = ($reparaciones_realizadas_semana_actual != 0) ? 100 : 0;
        }
    ...
    {
        $nuevos_clientes = DB::select('SELECT COUNT(*) as cuenta FROM reparacion WHERE fh_inicio BETWEEN DATE_SUB(NOW(), INTERVAL 1 day) AND NOW();')[0]->cuenta;
        ...
        if ($nuevos_clientes_1_dia != 0) {
            $porcentaje_nuevos_clientes = (($nuevos_clientes - $nuevos_clientes_1_dia) / $clientes) * 100;
        } else {
            $porcentaje_nuevos_clientes = ($nuevos_clientes != 0) ? 100 : 0; // O cualquier valor predeterminado que desees asignar
        }
    ...
    public function ventas()
    {
        ...
        if ($ventas_1_dia != 0) {
            ...
            $porcentaje_ventas = ($ventas_del_dia) ? 100 : 0; // O cualquier valor predeterminado que desees asignar
        }
